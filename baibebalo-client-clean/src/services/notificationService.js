...
/**
 * Vérifier si on est dans Expo Go (où les notifications push ne fonctionnent pas)
 */
const isExpoGo = () => {
  try {
    // Expo Go a certaines constantes spécifiques
    return Constants?.executionEnvironment === 'storeClient' || 
           Constants?.appOwnership === 'expo';
  } catch {
    // Si Constants n'est pas disponible, on assume que c'est peut-être Expo Go
    // On essaiera et on gérera l'erreur si ça échoue
    return false;
  }
};
  ...
  try {
    ...
    try {
      ...
      if (errorMessage.includes('Expo Go') || 
        ...
        // Ne pas afficher de warning - c'est normal en Expo Go
        return null;
      }
