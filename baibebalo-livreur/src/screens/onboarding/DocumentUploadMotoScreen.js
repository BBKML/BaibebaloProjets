...
const documents = [
  ...
  { id: 'permis_recto', label: 'Permis de conduire - Recto', icon: 'car-outline' },
  { id: 'permis_verso', label: 'Permis de conduire - Verso', icon: 'car-outline' },
  { id: 'carte_grise_recto', label: 'Carte grise - Recto', icon: 'document-text-outline' },
  { id: 'carte_grise_verso', label: 'Carte grise - Verso', icon: 'document-text-outline' },
  { id: 'assurance', label: 'Attestation assurance', icon: 'shield-checkmark-outline' },
  { id: 'photo_moto', label: 'Photo de la moto (plaque visible)', icon: 'bicycle' },
];

export default function DocumentUploadMotoScreen({ navigation }) {
  ...
  const [vehiclePlate, setVehiclePlate] = useState(registrationData.vehiclePlate || '');
  ...
  const isValidPlate = () => {
    return vehiclePlate.trim().length >= 3;
  };
  ...
  const handleContinue = async () => {
    ...
    if (!isValidPlate()) {
      Alert.alert('Plaque manquante', 'Veuillez saisir le numéro de plaque de votre moto');
      return;
    }
    
    await updateRegistrationData({ vehiclePlate: vehiclePlate.trim().toUpperCase() });
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        <Text style={styles.subtitle}>
          Livreur Moto - Uploadez tous les documents requis
        </Text>
        ...
        <View style={styles.plateInputContainer}>
          <Text style={styles.plateLabel}>Numéro de plaque d'immatriculation *</Text>
          <TextInput
      ...
      <View style={styles.bottomContainer}>
        <TouchableOpacity 
          style={[
            ...
            (!isAllUploaded() || !isValidPlate()) && styles.primaryButtonDisabled
          ]}
          onPress={handleContinue}
          disabled={!isAllUploaded() || !isValidPlate()}
        >
